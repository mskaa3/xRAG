...
llm_tokenizer = AutoTokenizer.from_pretrained(
    llm_name_or_path, add_eos_token=False, use_fast=False, padding_side="left"
)
...
retriever = (
    SFR.from_pretrained(retriever_name_or_path, torch_dtype=torch.bfloat16)
    .eval()
    .to(device)
)
...
def download(
    source: str, destination: str = None, verbose: bool = False, exclude: str = None
) -> str:
    if not is_rclone_installed():
        raise RuntimeError("rclone is not installed")
...
def process(
    ds,
    max_tokens,
    xrag_token: bool = True,
    retriever_max_length: int = 32000,
    ...
    if xrag_token:
        ...
        prompts = [
            rag_template.format_map({"question": q, "document": XRAG_TOKEN})
            for q in questions
        ]
    else:
        prompts = [
            rag_template.format_map({"question": q, "document": d})
    ...
    suffix = "_no_token" if not xrag_token else ""
    file_name = file_name + suffix

    if save_format == "csv":
        print(f"saving to results/{file_name}.csv")
        df.to_csv(f"results/{file_name}.csv", index=False, encoding="utf-8", sep=";")
    elif save_format == "jsonl":
        ds = Dataset.from_pandas(df)
        print(f"saving to results/{file_name}.jsonl")
        with open(f"results/{file_name}.jsonl", "w", encoding="utf-8") as outfile:
